...
| Session Local Coordinate System | 以本次校正時 user 面向的出拳方向建立的座標系統。 |
...
- 新增 `punch_trajectory` version 2 Analysis Specification 與 Production Executor，以左右手腕 Common IMU CSV 產生逐拳相對三維軌跡。
- 沿用 BAP 既有的 IMU 探索、左右手腕分配、Session 上傳、Backend Job 與 Result 查詢流程。
- 正式測量前要求 user 面向出拳方向並保持準備姿勢，完成靜止校正後再輸入錄製時間及開始測量。
...
- `punch-trajectory-analysis`: 定義出拳軌跡的校正、左右手腕輸入、Backend 軌跡重建、Result schema，以及 Desktop 互動式 3D 顯示行為。

### Modified Capabilities

- `desktop-app-shell`: 將具有 version 2 Production Executor 的出拳軌跡入口從待開發改成可完成端到端測量與查看 Result。
- `analysis-specification-contract`: 加入 `punch_trajectory` version 2 的 Input Roles、校正參數與明確 Result schema。
...
- Desktop App 將新增校正說明、正式錄製轉場、軌跡 Result view 與互動式 3D Widget，並提升 Desktop version。
