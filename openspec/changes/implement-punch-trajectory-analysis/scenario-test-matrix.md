...
|---|---|---|
| analysis-specification-contract | 前後端使用相同的 version 3 | `tests/backend/test_punch_trajectory.py::test_contract_rejects_placeholder_and_inconsistent_counts` |
| analysis-specification-contract | version 3 收到校正參數 | `tests/backend/test_punch_trajectory.py::test_contract_rejects_placeholder_and_inconsistent_counts` |
...
| desktop-app-shell | 出拳軌跡 version 3 可執行 | `tests/backend/test_analysis_registry.py::test_default_production_registry_exposes_direct_trajectory_version_three` |
| desktop-app-shell | 完成待開發項目的 IMU 來源選擇 | `tests/desktop/test_desktop_ui_design.py::test_unavailable_capability_does_not_record` |
| desktop-app-shell | 出拳軌跡 Executor 可用 | `tests/backend/test_analysis_registry.py::test_default_production_registry_exposes_direct_trajectory_version_three` |
...
| punch-trajectory-analysis | user 準備正式錄製 | `tests/desktop/test_desktop_ui_design.py::test_trajectory_ready_state_skips_calibration` |
| punch-trajectory-analysis | user 開始出拳軌跡測量 | `tests/desktop/test_analysis_recording.py::test_trajectory_version_three_starts_without_calibration` |
| punch-trajectory-analysis | 左右手 IMU 初始方向不同 | `tests/backend/test_punch_trajectory.py::test_direct_recording_turns_inconsistent_initial_direction_into_warning` |
...
| punch-trajectory-analysis | 正式資料不足 | `tests/backend/test_punch_trajectory.py::test_insufficient_calibration_becomes_safe_contract_error` |
