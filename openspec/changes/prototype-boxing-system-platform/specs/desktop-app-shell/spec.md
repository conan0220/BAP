...
| 拳擊測量項目 | 出拳次數、出拳速度、出拳力量、出拳軌跡及拳型辨識。 |
| 待開發 | 畫面入口已存在，但本次不提供資料錄製或分析結果。 |
...
Desktop App MUST 將出拳次數、出拳速度、出拳力量、出拳軌跡及拳型辨識分成五個獨立入口，不得提供同時選擇兩個以上項目的量測流程。
...
- **AND** Desktop App 不顯示力量、速度、次數、軌跡或拳型結果
