...
| 出拳力量 | 使用沙袋上、下方兩顆 IMU 估算單次最大打擊力與打擊位置的分析項目。 |
...
#### Scenario: 出拳力量 version 1 可執行
- **WHEN** Backend 回報 `punch_force` version 1 已有可執行的 Production Executor
- **THEN** 出拳力量入口顯示為可使用
- **AND** user 能進入該項目的 IMU 探索、沙袋設定、校正、正式錄製及 Result flow
...
#### Scenario: 出拳力量 Executor 可用
- **WHEN** user 進入出拳力量，且 Backend 回報 `punch_force` version 1 可執行
- **THEN** Desktop App 允許 user 完成沙袋設定、校正與正式測量流程
- **AND** Desktop App 只顯示 Backend 實際回傳且通過契約驗證的力量 Result
