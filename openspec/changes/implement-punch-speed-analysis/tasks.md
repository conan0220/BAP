...
- [ ] 8.1 使用一個無線接收器與左右手兩顆 IMU，確認校正期間保持靜止時不會產生出拳明細，轉動手腕後回到靜止也不會出現持續增加的速度。
- [ ] 8.2 由 user 依序完成慢速、一般速度與快速 Shadow boxing，確認左右手歸屬正確，且 UI 顯示的拳頭速度相對順序合理。
- [ ] 8.3 由 user 驗證 Session 正常結束、提前結束、重新測量與必要 Node 中斷行為，並註記本次人工驗證沒有外部 Ground Truth，未宣稱絕對速度誤差範圍。
