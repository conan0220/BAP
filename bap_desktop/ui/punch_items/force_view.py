...
class MatplotlibForceCanvas:
    def __init__(self, result: dict) -> None:
        ...
        axes[0].plot(time, [point["top_horizontal_acceleration_mps2"] for point in points], label="沙袋上方")
        axes[0].plot(time, [point["bottom_horizontal_acceleration_mps2"] for point in points], label="沙袋下方")
        ...
        axes[1].set_ylabel("角加速度 (rad/s²)")
        axes[1].legend(loc="upper right")
        force = [point["force_kgf"] for point in points]
        axes[2].plot(time, force, color="#c72538", label="估算力量")
        ...
        axes[2].scatter([time[peak_index]], [force[peak_index]], color="#c72538", zorder=3, label="峰值")
        axes[2].set_ylabel("力量 (kgf)")
        axes[2].set_xlabel("正式錄製經過時間 (秒)")
