...
class AnrotSerialParser:
    ...
    def _parse_hi63(self, data, ofs):
        """Parse 0x63 gateway compact aggregated packet.

        Packet layout (matching C++ id0x63_header_t + id0x63_node_t):
          Header (8 bytes): [0x63][GWID][node_cnt][rev][ts_ms (uint32 LE)]
          Node   (34 bytes each): [0x93][ID][acc x3][mag x3][gyr x3][quat x4][eul x3]
          All sensor values are int16 LE with fixed-point scale factors.

        Returns:
            (new_offset, list_of_AnrotFrame)
        """
        node_frames = []

        if ofs + 8 > len(data):
            return len(data), node_frames

        gw_id = data[ofs + 1]
        node_cnt = data[ofs + 2]
        gw_ts_ms = struct.unpack_from('<I', data, ofs + 4)[0]
        ofs += 8

        for i in range(min(node_cnt, 16)):
            if ofs + 34 > len(data):
                break

            frame = AnrotFrame()
            frame.frame_type = 0x63
            frame.gw_id = gw_id
            frame.node_id = data[ofs + 1]
            frame.gw_ts_ms = gw_ts_ms
            frame.system_time_ms = gw_ts_ms
            frame.node_count = node_cnt
            frame.node_index = i

            # Acceleration: int16 x3 / 1000 -> g
            raw_acc = struct.unpack_from('<3h', data, ofs + 2)
            frame.acc = tuple(x / 1000.0 for x in raw_acc)

            # Magnetometer: int16 x3 / 10 -> uT
            raw_mag = struct.unpack_from('<3h', data, ofs + 8)
            frame.mag = tuple(x / 10.0 for x in raw_mag)

            # Angular velocity: int16 x3 / 10 -> deg/s
            raw_gyr = struct.unpack_from('<3h', data, ofs + 14)
            frame.gyr = tuple(x / 10.0 for x in raw_gyr)

            # Quaternion: int16 x4 / 32767 -> normalized
            raw_quat = struct.unpack_from('<4h', data, ofs + 20)
            frame.quat = tuple(x / 32767.0 for x in raw_quat)

            # Euler angles: int16 x3 / 100 -> deg (roll, pitch, yaw)
            raw_eul = struct.unpack_from('<3h', data, ofs + 28)
            frame.roll = raw_eul[0] / 100.0
            frame.pitch = raw_eul[1] / 100.0
            frame.yaw = raw_eul[2] / 100.0

            node_frames.append(frame)
            ofs += 34

        return ofs, node_frames

    def parse_data(self, data):
        """Parse data. Returns list of extra frames for gateway packets (e.g. 0x63)."""
        ofs = 0
        extra_frames = []
        while ofs < len(data):
            item_type = data[ofs]
            if item_type == FRAME_TAG_HI63:
                new_ofs, node_frames = self._parse_hi63(data, ofs)
                extra_frames.extend(node_frames)
                ofs = new_ofs
            else:
    ...
    def print_parsed_data(data):
        ...
        if data.frame_type is not None:
            data_fields = [
                ...
                ("Gateway ID",             f"{data.gw_id}" if data.gw_id is not None else None),
                ("Node ID",                f"{data.node_id}" if data.node_id is not None else None),
                ("Node",                   f"{data.node_index + 1}/{data.node_count}" if data.node_count is not None else None),
                ("GW Timestamp (ms)",      f"{data.gw_ts_ms}" if data.gw_ts_ms is not None else None),
...
if __name__ == "__main__":
    ...
    for i, frame in enumerate(frames_hi91):
        AnrotSerialParser.print_parsed_data(frame)
