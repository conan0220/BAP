...
def cmd_read(port, baudrate):
    ...
    serial_parser = AnrotSerialParser()
    nmea_parser = AnrotNmeaParser()
        ...
        with serial.Serial(port, int(baudrate), timeout=1) as ser:
            while True:
                if ser.in_waiting:
                    ...
                    try:
                        anrot_frames = serial_parser.parse(data)
                        nmea_frames = nmea_parser.parse(data.decode('ascii', errors='ignore'))
                        
                        frame_count += len(anrot_frames) + len(nmea_frames)
                        
                        if anrot_frames:
                            latest_anrot_frame = anrot_frames[-1]
                        if nmea_frames:
                        ...
                        if current_time - last_display_time >= display_interval:
                            ...
                            if latest_anrot_frame:
                                serial_parser.print_parsed_data(latest_anrot_frame)
